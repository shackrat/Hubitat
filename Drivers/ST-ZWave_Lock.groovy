...
def zwaveEvent(UserCodeReport cmd) {
	...
	if (codeID.toInteger() == state.checkCode) {  // getLockCodes() was called, keep requesting the codes in order
...
private Map lockCodesEvent(lockCodes) {
	createEvent(name: "lockCodes", value: lockCodes.toJson, displayed: false,
 ...
 */
def isSchlageLock() {
	// zwaveInfo not populating for the FE599, so manually populate
